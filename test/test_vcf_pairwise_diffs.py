...
import numpy as np
import pysam
from dnaUtilsPy.vcf import pairwise_diffs
...
def test_vcf_pairwise_diffs():
    ...
    pairdiffs = pairwise_diffs(
        os.path.join(dired, "test.vcf"))
    pairdiffs2 = _py_pairwise_diffs(
        os.path.join(dired, "test.vcf"))
    for k in ('n_comparisons', 'n_differences'):
        pairdiffs[k] = pairdiffs[k].tolist()
        pairdiffs2[k] = pairdiffs2[k].tolist()

    pprint.pprint(pairdiffs)

    assert pairdiffs == pairdiffs2
    with open(os.path.join(dired, "test_vcf_pairwise_diffs.out")) as f:
        assert ast.literal_eval(f.read()) == pairdiffs
